...
EcgAnnotation::EcgAnnotation(PANNHDR p): _ann(nullptr), _annNum(0), _qrsAnn(nullptr),
                _qrsNum(0), _auxNum(0), _aux(nullptr)
{
        if (p) {
                memcpy(&_ahdr, p, sizeof(ANNHDR));
        } else { //defaults
                _ahdr.minbpm = 40;       //min bpm
                _ahdr.maxbpm = 200;      //max bpm
                _ahdr.minQRS = 0.04;     //min QRS duration
                _ahdr.maxQRS = 0.2;      //max QRS duration
                _ahdr.qrsFreq = 13.0;    //QRS filtration frequency
                _ahdr.ampQRS = INTER1;   //inter1 filter
                _ahdr.minUmV = 0.2;      //min UmV of R,S peaks
                _ahdr.minPQ = 0.07;      //min PQ duration
                _ahdr.maxPQ = 0.20;      //max PQ duration
                _ahdr.minQT = 0.21;      //min QT duration
                _ahdr.maxQT = 0.48;      //max QT duration
                _ahdr.pFreq = 9.0;       //cwt Hz for P wave
                _ahdr.tFreq = 3.0;       //cwt Hz for T wave
                _ahdr.biTwave = NORMAL;  //normal wave                
        }
}
...
int** EcgAnnotation::GetQRS(const double *data, int size, double sr, wchar_t *fltdir)
{
        ...
        double eCycle = (60.0 / (double)_ahdr.maxbpm) - _ahdr.maxQRS;  //secs
        if (int(eCycle*sr) <= 0) {
                eCycle = 0.1;
                _ahdr.maxbpm = int(60.0 / (_ahdr.maxQRS + eCycle));
        }
        //////////////////////////////////////////////////////////////////////////////
        ...
        for (int m = add; m < size; m++) {                  //MAX 200bpm  [0,3 sec  min cario size]
                m += int(_ahdr.maxQRS * sr);                     //smpl + 0,20sec    [0,20 max QRS length]
                ...
                if ((m - add + 1) - QRS[QRS.size()-1] > _ahdr.minQRS*sr)  //QRS size > 0.04 sec
        ...
        if (_qrsNum > 0)                              //         46: ?
        {                                           //          1: N    -1: nodata in **AUX
                _qrsAnn = new int*[2*_qrsNum];                    // [samps] [type] [?aux data]
...
bool EcgAnnotation::Filter30hz(double *data, int size, double sr, wchar_t *fltdir) const
{
        ...
        double *pspec = cwt.CwtTrans(data, _ahdr.qrsFreq);      //10-13 Hz transform?
        for (int i = 0; i < size; i++)
                data[i] = pspec[i];
        cwt.closeCwt();
...
void EcgAnnotation::GetEctopics(int **ann, int qrsnum, double sr) const
{
        ...
        for (int n = -2; n < (int)RRs.size() - 2; n++) {
                ...
                if (60.0 / rr1 < _ahdr.minbpm || 60.0 / rr1 > _ahdr.maxbpm) //if RR's within 40-200bpm
                        continue;
                if (60.0 / rr2 < _ahdr.minbpm || 60.0 / rr2 > _ahdr.maxbpm) //if RR's within 40-200bpm
                        continue;
                if (60.0 / rr3 < _ahdr.minbpm || 60.0 / rr3 > _ahdr.maxbpm) //if RR's within 40-200bpm
                        continue;
...
int** EcgAnnotation::GetPTU(const double *data, int length, double sr, wchar_t *fltdir, int **ann, int qrsnum)
{
        ...
        for (int n = 0; n < qrsnum - 1; n++) {
                ...
                for (int i = maNum; i < (int)_ma.size(); i++) {
                        if (_ma[i] > ann[n*2+1][0] && _ma[i] < ann[n*2+2][0]) {
                                Pwave.push_back(0);
                                Pwave.push_back(0);
                                Pwave.push_back(0);
                                Twave.push_back(0);
                ...
                if (60.0 / rr < _ahdr.minbpm || 60.0 / rr > _ahdr.maxbpm - 20) { //check if normal RR interval (40bpm - 190bpm)
                        Pwave.push_back(0);
                        Pwave.push_back(0);
                        Pwave.push_back(0);
                        Twave.push_back(0);
                ...
                if (sr*_ahdr.maxQT - (ann[n*2+1][0] - ann[n*2+0][0]) > size - add)
                        size = size - add;
                else
                        size = sr * _ahdr.maxQT - (ann[n*2+1][0] - ann[n*2+0][0]) - add;
                ...
                if (_ahdr.biTwave == BIPHASE)
                        cwt.initCwt(size, CWT::GAUS, 0, sr);                 //5-Gauss wlet
                else
                        cwt.initCwt(size, CWT::GAUS1, 0, sr);                //6-Gauss1 wlet

                pspec = cwt.CwtTrans(data + annPos + add, _ahdr.tFreq);//,false,lvl,rvl);   //3Hz transform  pspec = size-2*add
                ...
                if (twave) {
                        if ((double)(T2 - T1) >= 0.09*sr) { // && (double)(T2-T1)<=0.24*sr)   //check for T wave duration
                                ...
                                if ((double)(T2 - ann[n*2+0][0]) >= _ahdr.minQT*sr && (double)(T2 - ann[n*2+0][0]) <= _ahdr.maxQT*sr)
                                        twave = true;
                                else
                                        twave = false;
                        } else
                ...
                cwt.initCwt(size, CWT::GAUS1, 0, sr);                                        //6-Gauss1 wlet
                pspec = cwt.CwtTrans(data + annPos + size23, _ahdr.pFreq);//,false,lvl,rvl);    //9Hz transform  pspec = size-2/3size
                ...
                if (pwave) {
                        if ((double)(P2 - P1) >= 0.03*sr && (double)(P2 - P1) <= 0.15*sr) { //check for P wave duration  9Hz0.03 5Hz0.05
                                ...
                                if ((double)(ann[n*2+2][0] - P1) >= _ahdr.minPQ*sr && (double)(ann[n*2+2][0] - P1) <= _ahdr.maxPQ*sr)
                                        pwave = true;
                                else
                                        pwave = false;
                        } else
        ...
        if (enoise.LFDenoise()) {
                ...
                for (int n = 0; n < qrsnum; n++) {
                        ...
                        Q = -1;
                        FindRS(pbuff, size, R, S, _ahdr.minUmV);
                        if (R != -1) R += annPos;
                        if (S != -1) S += annPos;
        ...
        {                                           //44-(t 45-t) 27-Twave
                ...
                for (int i = 0; i < (int)Twave.size(); i += 3) {   //Twave=Pwaves=qrsPeaks size
                        //QRS complex
                        _ann[index][0] = ann[qindex][0];           //(QRS
                        _ann[index][1] = ann[qindex][1];           //
                        _ann[index++][2] = ann[qindex++][2];       //aux
                        if (qrsPeaks[i]) { //q
                        ...
                        if (!Twave[i] && !Pwave[i]) {          //check for MA noise
                                for (int m = maNum; m < (int)_ma.size(); m++) {
                                        if (_ma[m] > ann[qindex-1][0] && _ma[m] < ann[qindex][0]) {
...
bool EcgAnnotation::SaveAnnotation(const wchar_t *name, int **ann, int nums)
{
        ...
        WriteFile(_fp, buff, 6, &bytes, 0);
        type = ann[0][1];
        anncode |= (type << 10);
        sprintf(buff, "%c%c", (char)LOBYTE(anncode), (char)HIBYTE(anncode));
        WriteFile(_fp, buff, 2, &bytes, 0);
...
bool EcgAnnotation::GetRRseq(int **ann, int nums, double sr, vector<double> *RR, vector<int> *RRpos) const
{
        ...
        for (int i = 0; i < nums; i++) {
                ...
                if (add != -1) {
                        ...
                        rr = 60.0 / ((r2 - r1) / sr);
                        if (rr >= _ahdr.minbpm && rr <= _ahdr.maxbpm) {
