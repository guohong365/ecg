...
{
public:
        EcgAnnotation(PANNHDR p = nullptr);
...
private:
        ...
        ANNHDR _ahdr;                    //annotation ECG params

        int **_ann, _annNum;              //annotation after getPTU (RR's + PT)
        int **_qrsAnn, _qrsNum;           //annotation after getQRS (RR's+MAnoise)  +ECT after getECT()
        vector <int> _ma;                //MA noise
        int _auxNum;
